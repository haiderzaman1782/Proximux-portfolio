export function ProcessCard({ step, icon, title, description, output }: { step: number, icon: React.ReactNode, title: string, description: string, output: string }) {
  return (
    <div className="text-center group">
      <div className="relative inline-flex items-center justify-center mb-6 w-14 h-14 sm:w-16 sm:h-16 rounded-full bg-[var(--accent-tint)] border border-[var(--accent-tint-strong)] text-[var(--saas-lime)] transition-all group-hover:scale-110 group-hover:bg-[var(--accent-tint-strong)] shadow-tint-sm">
        {icon}
